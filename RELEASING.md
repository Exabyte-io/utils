...
`github:mat3ra/utils#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/utils/releases/download/wip-<short-sha>/utils.tgz
```
...
```bash
npm run mat3ra:install -- utils wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/utils@<release-url>` with
